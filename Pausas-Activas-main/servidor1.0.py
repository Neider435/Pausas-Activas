...
def estado():
    try:
        ...
        cumpleaneros_hoy = []
        try:
            with open("cumpleanos.json", "r", encoding='utf-8') as f:
        ...
        if "cumpleanos" in horarios_hoy and cumpleaneros_hoy:
            for evento in horarios_hoy["cumpleanos"]:
                ...
                if inicio_segundos <= ahora_segundos <= fin_segundos:
        ...
        if "anuncios_video" in horarios_hoy:
            for evento in horarios_hoy["anuncios_video"]:
                ...
                if inicio_segundos <= ahora_segundos <= inicio_segundos + duracion_evento:
                    return jsonify({
                        "activo": True,
                        "tipo": "anuncios_video",
                ...
                for evento in lista_pausa:
                    ...
                    if inicio_segundos <= ahora_segundos <= inicio_segundos + duracion_evento:
                        return jsonify({
                            ...
                            "tipo": "pausas_activas",
                            "archivo": evento["archivo"],
                            "url": f"https://www.youtube.com/watch?v={evento['archivo']}",
                            "duracion": duracion_evento
                        })

        # --- Si no hay contenido programado ---
